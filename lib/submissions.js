...
const DEFAULT_DATA = { participants: [], assignments: {}, responses: [] };
...
/**
 * Remove stale assignments from an in-memory data object.
 * Caller is responsible for persisting this data.
 *
 * @param {object} data - The in-memory submissions data object
 * @param {number} maxAgeMs - Maximum assignment age in ms (default: 24 hours)
 * @returns {number} Number of removed stale assignments
 */
function cleanupStaleAssignmentsLocked(data, maxAgeMs = 24 * 60 * 60 * 1000) {
  const now = Date.now();
  let removed = 0;
 ...
 *
 * Uses the same file lock path as assignment/submission writes to avoid races.
 *
 * @param {number} maxAgeMs - Maximum assignment age in ms (default: 24 hours)
 * @returns {Promise<number>} Number of removed stale assignments
 */
async function cleanupStaleAssignments(maxAgeMs = 24 * 60 * 60 * 1000) {
  return withLock(() => {
    const data = read();
    const removed = cleanupStaleAssignmentsLocked(data, maxAgeMs);
